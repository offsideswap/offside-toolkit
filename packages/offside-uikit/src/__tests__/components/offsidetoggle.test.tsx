...
it("renders correctly", () => {
  const { asFragment } = renderWithTheme(<OffsideToggle checked onChange={handleChange} scale="md" />);
...
it("renders correctly scale sm", () => {
  const { asFragment } = renderWithTheme(<OffsideToggle checked onChange={handleChange} scale="sm" />);
  expect(asFragment()).toMatchInlineSnapshot(`
    <DocumentFragment>
      .c0 {
      position: relative;
